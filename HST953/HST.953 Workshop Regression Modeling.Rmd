...
#setwd("~/data/HST.953") 
# Students will need to set their own path or see below

if(!("Hmisc" %in% installed.packages()[,1])) {install.packages("Hmisc")}
library(Hmisc)
...
We can do hypothesis testing using the `anova` function and an F-test comparing the nested models.  First `bun.creat.renal.con.lm` and `bun.creat.renal.int.lm`:
...
Here, there does appear to be statistically significant evidence that the renal effect is non-zero. #partial F-test#
...
```{r}
plot(log(dat$bun_first),-1/dat$creatinine_first^(1/3),pch=19,col=dat$renal_flg)
abline(a=coef(bun.creat.renal.con.lm)[1],b=coef(bun.creat.renal.con.lm)[2],col="blue",lwd=2)
abline(a=sum(coef(bun.creat.renal.con.lm)[c(1,3)]),b=coef(bun.creat.renal.con.lm)[2],col="red",lwd=2)
```
...
```
### The median levels of the sofa score in MED, SURG apperears to be higher than those of NMED,NSURG and TRAUM. 
...
```
### "MED" has a sginififantly higher median level of sofa socore ( Risk ratio; -0.95(-1.22, -0.67), p<0.0001), compaered to "TRAUMA"
...
```
### From the result above, there is a significantly dirrence of the mean SOFA score amomg all service units.
...
```{r}
class(dat$bun_first)
summary(dat$bun_first)
dim(dat)
length(dat$bun_first)

```
anova(sofa.serviceunit2.lm,sofa.serviceunit2.logbun.lm)
```{r}
sofa.serviceunit2.lm <- lm(I(dat$sofa_first) ~ I(dat$service_unit2),data=dat)
sofa.serviceunit2.logbun.lm <- lm(I(dat$sofa_first) ~ I(dat$service_unit2) + log(bun_first), data = dat)

drop1(sofa.serviceunit2.lm)
...
drop1(aline.sofa.glm,test = "Chisq")
```
...
##Why we need "dat$" here? why only"age.cut" dose not function? like age.cat <- cut2(age,c(50,60,70,80))
...
Let's continue $ by adding `age.cat`in the model$, and repeating the previous step, but this time using our new model with `aline_flg` and `age.cat`.
...
Here, we choose to add @`service_unit2` in the model@, but could have also included `stroke_flg`.
...
```
### In the final model above adjusted for age.cat, sofa_cat, service_unit2, chf_flg, cad_flg, stroke_flg, mal_flg, resp_flg, aline_flg is not significantly associated with the hospiral mortality. 
